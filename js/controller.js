  ...
  Html5Skin.prototype = {
    ...
    subscribeBasicPlaybackEvents: function () {
      if(!this.state.isSubscribed) {
        ...
        this.mb.subscribe(OO.EVENTS.VOLUME_CHANGED, "customerUi", _.bind(this.onVolumeChangedToggleClosedCaptions, this));
    ...
    onVolumeChangedToggleClosedCaptions: function (event, newVolue) {
      ...
      if (isAutoUpNext){
        ...
        if (selectedContentData.clickedVideo.embed_code){
          this.mb.publish(OO.EVENTS.SET_EMBED_CODE, selectedContentData.clickedVideo.embed_code,
            {"autoplay": !!(this.state.config.nym && this.state.config.nym.autoPlayNextVideo)});
        }
